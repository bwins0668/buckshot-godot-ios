...
# Phase v0.8.3 — split sim/device cmdline overrides via env vars:
#   PATCH_GODOT_CMDLINE_DEVICE  → applied here (default: empty = no override;
#                                  Godot 4.3 default vulkan→MoltenVK→Metal
#                                  works on iPad M2 + iOS 27).
#   PATCH_GODOT_CMDLINE_SIM     → applied in ios.yml "Simulator smoke test"
#                                  step (default: opengl3 = the only viable
#                                  sim driver — see patch_info_plist.py).
...
if [ -f "$PATCHED_PLIST" ]; then
  python3 "$GITHUB_WORKSPACE/ci/patch_info_plist.py" --device "$PATCHED_PLIST"
else
